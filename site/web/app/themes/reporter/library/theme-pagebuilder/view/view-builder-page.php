...
switch($action) {
	...
	case 'create' :
		...
		if(!is_wp_error($new_id)) {
			...
			$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'engine') . '<strong>' . $template_name . '</strong>' . __(' page template has been successfully created', 'engine') . '</p></div>';
		} else {
			$errors = '<ul>';
			...
			$messages[] = '<div id="message" class="error"><p>' . __('Sorry, the operation was unsuccessful for the following reason(s): ', 'engine') . '</p>' . $errors . '</div>';
		}
	...
	case 'update' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __('The ', 'engine') . '<strong>' . $template_name . '</strong>' . __(' page template has been updated', 'engine') . '</p></div>';
		break;
	...
	case 'delete' :
		...
		$messages[] = '<div id="message" class="updated"><p>' . __('The template has been successfully deleted', 'engine') . '</p></div>';
		break;
}
	...
	<div id="page-builder-frame">
		...
		<div id="page-builder-column" class="metabox-holder <?php echo $disabled ?>">
			<div id="page-builder-archive" class="postbox">
				<h3 class="hndle"><span><?php _e('Available Blocks', 'engine') ?></span><span id="removing-block"><?php _e('Deleting', 'engine') ?></span></h3>
				<div class="inside">
					<ul id="blocks-archive" class="cf">
						<?php $this->blocks_archive() ?>
					</ul>
					<p><?php _e('Need help? Use the Help tab in the upper right of your screen.', 'engine') ?></p>
				</div>
			</div>
		</div>
					...
					<form id="update-page-template" action="<?php echo $this->args['page_url'] ?>" method="post" enctype="multipart/form-data">
								...
								<div id="submitpost" class="submitbox">
									<div class="major-publishing-actions cf">
										...
										<label class="open-label" for="template-name">
											<span><?php _e('Template Name', 'engine') ?></span>
										...
										<div class="publishing-action">
											<?php submit_button( empty( $selected_template_id ) ? __( 'Create Template', 'engine' ) : __( 'Save Template', 'engine' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_header' ) ); ?>
						...
						<div id="aqpb-body">
							...
							<ul class="blocks cf" id="blocks-to-edit">
								...
								if($selected_template_id === 0) {
									...
									<br/><br/>
									When you have finished building your custom page template, make sure you click the Save Template button.', 'engine');
									echo '</p>';
						...
						<div id="aqpb-footer">
							<div class="major-publishing-actions cf">
								<div class="publishing-action">
									<?php if(!empty($selected_template_id)) {
										submit_button( __( 'Save Template', 'engine' ), 'button-primary ', 'save_template', false, array( 'id' => 'save_template_footer' ) ); 
									} ?>
								</div><!-- END .publishing-action -->
							</div><!-- END .major-publishing-actions -->
						</div>
