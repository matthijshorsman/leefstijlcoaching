...
function optionsframework_options_page(){
	...
	/*
	//for debugging
	$reporter_data = get_option(OPTIONS);
	print_r($reporter_data);
	*/	
...
function of_load_only() 
{
	...
	//custom js
	wp_enqueue_script('smof_palette', ADMIN_DIR .'assets/js/smof_palette.js', array( 'jquery' ));
	wp_enqueue_script('smof_webfonts', ADMIN_DIR .'assets/js/smof_webfonts.js', array( 'jquery' ));
	...
	wp_localize_script( 'smof', 'mtvars', $params );
...
function of_ajax_callback() 
{
	...
	elseif($save_type == 'restore_options')
	{
		...
		$mt_options_data = get_option(BACKUPS);
		
		update_option(OPTIONS, $mt_options_data);
		generate_options_css($mt_options_data);
		
		die('1'); 
	}
	elseif($save_type == 'import_options'){
			
		$mt_options_data = $_POST['data'];
		$mt_options_data = unserialize(base64_decode($mt_options_data)); //100% safe - ignore theme check nag
		update_option(OPTIONS, $mt_options_data);
		generate_options_css($mt_options_data);
	...
	{
		wp_parse_str(stripslashes($_POST['data']), $mt_options_data);
		unset($mt_options_data['security']);
		unset($mt_options_data['of_save']);
		update_option(OPTIONS, $mt_options_data);
		generate_options_css($mt_options_data);
