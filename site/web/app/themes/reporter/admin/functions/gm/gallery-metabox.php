...
class BE_Gallery_Metabox
{
	...
	public function admin_scripts() {
		...
		wp_register_script( 'gallery-metabox-ajax', get_template_directory_uri() . '/admin/functions/gm/lib/js/gallery-metabox-ajax.js', array( 'jquery' ), null, true );
		wp_register_style( 'gallery-metabox-style', get_template_directory_uri() . '/admin/functions/gm/lib/css/gallery-metabox-style.css', array(), null, 'all' );
	...
	public function metabox_add() {
		...
		$post_types		= apply_filters( 'be_gallery_metabox_post_types', array( 'slider') );
		...
		foreach( $post_types as $post_type ) {
			...
			if( apply_filters( 'be_gallery_metabox_limit', true, $post_id ) ) {
				// Add Metabox
				add_meta_box( 'be_gallery_metabox', __( 'Slider Images', 'mthemes' ), array( $this, 'gallery_metabox' ), $post_type, $context, $priority );
	...
	public function gallery_display( $loop ) {
		...
		foreach( $loop as $image ):
			...
			// removal button
			$gallery .= apply_filters( 'be_gallery_metabox_remove', '<span class="be-image-remove" rel="' . $image->ID .'"><img src="' . get_template_directory_uri() . '/admin/functions/gm/lib/img/cross-circle.png'. '" alt="Remove Image" title="Remove Image"></span>' ); 
