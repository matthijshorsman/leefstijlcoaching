...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
	    ...
	    $engine_options_data = get_option(OPTIONS);
		...
		foreach ($options as $value) {
			...
			if ( $value['type'] != "heading" )
			{
				...
				if (array_key_exists("fold",$value)) {
					if ($engine_options_data[$value['fold']]) {
						$fold="f_".$value['fold']." ";
					} else {
				...
				$output .= '<div id="section-'. (isset($value['id']) ? $value['id'] : '').'" class="'.$fold.'section section-'.$value['type'].' '. $class .'">'."\n";
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					$t_value = '';
					$t_value = isset($engine_options_data[$value['id']]) ? stripslashes($engine_options_data[$value['id']]) : '';
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$option.'" ' . selected($engine_options_data[$value['id']], $option, false) . ' />'.$option.'</option>';	 
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = isset($engine_options_data[$value['id']]) ? stripslashes($engine_options_data[$value['id']]) : '';			
				...
				case "radio":
					 ...
					 foreach($value['options'] as $option=>$name) {
						$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($engine_options_data[$value['id']], $option, false) . ' /><label class="radio">'.$name.'</label><br/>';				
					}
				break;
				
				//checkbox option
				case 'checkbox':
					if (!isset($engine_options_data[$value['id']])) {
						$engine_options_data[$value['id']] = 0;
					}
					...
					$output .= '<input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($engine_options_data[$value['id']], 1, false) .' />';
				break;
				
				//multiple checkbox option
				case 'multicheck': 			
					$multi_stored = $engine_options_data[$value['id']];
				...
				//colorpicker option
				case 'color':		
					$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$engine_options_data[$value['id']].'"></div></div>';
					$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $engine_options_data[$value['id']] .'" />';
				break;
				
				//typography option	
				case 'typography':
				
					$typography_stored = isset($engine_options_data[$value['id']]) ? $engine_options_data[$value['id']] : $value['std'];
					...
					/* Google fonts */
					$hide = isset($typography_stored['face']) && $typography_stored['face'] == 'gfonts' ? 'no-hide' : 'hide';
					
					
					$gfont = isset($typography_stored['gfont']) ? $typography_stored['gfont'] : 'Open Sans';
					$lineheight = isset($typography_stored['height']) ? '&font_height='. $typography_stored['height'] : '';
					$color = isset($typography_stored['color']) ? '&font_height='. $typography_stored['color'] : '';
					$style = isset($typography_stored['style']) ? '&font_height='. $typography_stored['style'] : '';
					
					$output .= '<div id="' . $value['id'] . '_google_fonts" class="'.$hide.' typography-preview typography-gfonts">';
						$output .= '<div class="gfont-input">';
							$output .= '<span>'. __('Enter font name from the ', 'framework') . '<a href="http://www.google.com/webfonts" title="Google Webfonts" target="_blank">Google Webfonts directory</a></span><span class="nicerify cf">';
							$output .= '<input name="'.$value['id'].'[gfont]" id="'. $value['id'] .'_gfont" type="text" placeholder="Example: Open Sans:300" value="'. $gfont .'"></div>';
						$output .= '<div class="gfont-preview"><iframe src="'. get_template_directory_uri() .'/admin/front-end/webfonts.php?font_family=' . $gfont . $lineheight . $color . $style .'"></iframe></div>';
					$output .= '</div>';
				...
				case 'border':
					...
					/* Border Width */
					$border_stored = $engine_options_data[$value['id']];
				...
				case 'images':
					...
					$select_value = isset($engine_options_data[$value['id']]) ? $engine_options_data[$value['id']] : '';
				...
				//group header
				case "groupheader":
					$groupheader_text = $value['std'];
					$output .= '<div class="of-group-header"><span></span>'.$groupheader_text.'</div>';
				break;
				...
				case 'slider':
					...
					$slides = $engine_options_data[$value['id']];
					$count = count($slides);
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
				...
				case 'sorter':
					...
					$sortlists = isset($engine_options_data[$value['id']]) && !empty($engine_options_data[$value['id']]) ? $engine_options_data[$value['id']] : $value['std'];
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = '';
					$select_value = isset($engine_options_data[$value['id']]) ? $engine_options_data[$value['id']] : $value['std'];
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','mthemes').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'transfer':
					...
					$instructions = $value['desc'];
					$output .= '<textarea id="export_data" rows="8">'.base64_encode(serialize($engine_options_data)) /* 100% safe - ignore theme check nag */ .'</textarea>'."\n";
